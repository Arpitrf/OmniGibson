...
def custom_reset(env, robot, episode_memory=None, grasp_mode=None): 
    ...
    base_yaw = 0.0
    base_yaw = np.random.uniform(-7, 7)
    ...
    if grasp_mode == "front":
        ...
        base_pos = np.array([0.08, 0.1, 0.0])
        base_x_noise = np.random.uniform(-0.01, 0.01)
        base_y_noise = np.random.uniform(-0.02, 0.02)
...
def grasp_handle(grasp_mode):
    ...
    if grasp_mode == "front":
        target_pose_obj = np.array([
            [-0.09275438, -0.98759589,  0.1266925,   0.05943889],
            [-0.40653036,  0.15371515,  0.90061352, -0.93325959],
            [-0.90891677,  0.0320315,  -0.41574547,  0.2953502 ],
            [ 0.        ,  0.,          0.,          1.        ],
        ])
    elif grasp_mode == "top":
        # w.r.t object (top grasp)
        target_pose_obj = np.array([
            [-0.1592298,  -0.97974089, -0.12146468,  0.05155695],
            [-0.96546137,  0.12883274,  0.22646515, -0.94136439],
            [-0.20622853,  0.15332945, -0.96641599,  0.38947618],
            [ 0.        ,  0.,          0.,          1.        ],
        ])
    ...
    if grasp_mode == "top":
        target_pose_obj = np.array([
            [-0.12991022, -0.98733175, -0.09110077,  0.07377872],
            [-0.98581696,  0.11877064,  0.11856838, -0.93460444],
            [-0.10624623,  0.10521192, -0.9887579,   0.27488186],
            [ 0.        ,  0.,          0.,          1.        ],
        ])
        cabinet_pos_world, cabinet_orn_world = scene.object_registry("name", "drawer").get_position_orientation()
        cabinet_pose_world = np.eye(4)
...
save_folder = 'omnigibson/arnav_trial/close_drawer1'
...
while episode_number < init_episode_number + num_samples:
    ...
    drawer.joints["j_link_5"].set_pos(drawer.joints["j_link_5"].upper_limit - 0.09)
