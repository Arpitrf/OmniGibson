...
def custom_reset(env, robot, episode_memory=None, grasp_mode=None): 
    ...
    if grasp_mode == "side":
        base_yaw = np.random.uniform(-3, -3)
    base_yaw = 0
    ...
    base_pos = np.array([0.25, -0.4, 0.0])
    if grasp_mode == "front":
        base_x_noise = np.random.uniform(-0.05, 0.05)
        base_y_noise = np.random.uniform(-0.15, 0.05)
    if grasp_mode == "side":
        # base_pos = np.array([0.38, -0.8, 0.0])
        base_x_noise = np.random.uniform(-0.03, 0.03)
        base_y_noise = np.random.uniform(-0.04, 0.04)
...
def grasp_handle(grasp_mode):
    ...
    if grasp_mode == "side":
        ...
        target_pose_obj = np.array([
            [-0.301, -0.11,   0.947, -0.199],
            [ 0.952,  0.019,  0.305, -0.552],
            [-0.051,  0.994,  0.099,  0.012],
            [ 0.,     0.,     0.,     1.   ]
        ])
    elif grasp_mode == "front":
        # w.r.t object (front grasp)
        target_pose_obj = np.array([
            [-0.998, -0.038,  0.05,  -0.00],
            [ 0.046,  0.093,  0.995, -0.6],
            [-0.043,  0.995, -0.092, -0.012],
            [ 0.,     0.,     0.,     1.   ]
        ])
    ...
    if grasp_mode == "front":
        ...
        target_pose_obj = np.array([
            [-0.321, -0.104,  0.941, -0.103],
            [ 0.946,  0.019,  0.325, -0.551],
            [-0.052,  0.994,  0.092,  0.012],
            [ 0.,     0.,     0.,     1.   ]
        ])
        cabinet_pos_world, cabinet_orn_world = scene.object_registry("name", "fridge").get_position_orientation()
        cabinet_pose_world = np.eye(4)
...
save_folder = 'omnigibson/arnav_trial/open_fridge2'
...
fridge.links["left_door"].mass = 20.0
fridge.joints["j_left_door"].friction = 300.0
fridge.links["right_door"].mass = 10.0
fridge.joints["j_right_door"].friction = 500.0

writer = None
state = og.sim.dump_state(serialized=False)
grasp_modes = ["front", "side"]
