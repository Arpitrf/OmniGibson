...
def move_primitive(robot, action_traj, episode_memory=None, writer=None):
    for waypoint_num, action_wrt_world in enumerate(action_traj):
        ...
        for _ in range(50):
            og.sim.step()
        obs, _ = env.get_obs()
        video_img = obs[f"{robot.name}"][f"{robot.name}:eyes:Camera:0"]["rgb"][:, :, :3].numpy()
        global save_video
        if save_video:
            global data_collection_images
            data_collection_images.append(video_img)
...
def custom_reset(env, robot, episode_memory=None, grasp_mode=None): 
    ...
    base_yaw = -10.0
    # base_yaw = np.random.uniform(-4, 4)
    ...
    # base_pos = np.array([-0.15, 0.0, 0.0])
    base_pos = np.array([-1.0000e-01,  1.1581e-07,  5.0002e-04])
    if grasp_mode == "front":
        base_x_noise = np.random.uniform(-0.01, 0.01)
        base_y_noise = np.random.uniform(-0.02, 0.02)
    elif grasp_mode == "top":
        base_pos = np.array([0.07, 0.1, 0.0])
        base_x_noise = np.random.uniform(-0.01, 0.0)
        base_y_noise = np.random.uniform(-0.01, 0.01)
...
def grasp_handle(grasp_mode):
    ...
    if grasp_mode == "front":
        target_pose_obj = np.array([
            [-0.02087069, -0.99753451,  0.06700229,  0.00631299],
            [-0.38457578,  0.06987058,  0.92044531, -1.03611946],
            [-0.92285746, -0.00655713, -0.38508586,  0.26606254],
            [ 0.        ,  0.,          0.,          1.        ],
        ])
        # target_pose_obj = np.array([
        #     [-0.00851022, -0.99354183,  0.1131468,   0.00677354],
        #     [-0.27914591,  0.11101309,  0.95381007, -1.0403838 ],
        #     [-0.96021099, -0.02346733, -0.27828788,  0.26578307],
        #     [ 0.        ,  0.,          0.,          1.        ],
        # ])
        # target_pose_obj = np.array([
        #     [ 0.0201292,  -0.98982378,  0.14086768,  0.02102326],
        #     [-0.28357661,  0.12945761,  0.95017105, -1.05738117],
        #     [-0.9587383,  -0.05907296, -0.27808499,  0.27042363],
        #     [ 0.        ,  0.,          0.,          1.        ],
        # ])
    elif grasp_mode == "top":
        # w.r.t object (top grasp)
        target_pose_obj = np.array([
            [ 0.01099326, -0.99651224, -0.08271947, -0.00168484],
            [-0.99013039, -0.02240685,  0.13834645, -1.07429886],
            [-0.13971741,  0.08038218, -0.98692338,  0.3309561 ],
            [ 0.        ,  0.,          0.,          1.        ],
        ])
    ...
    if grasp_mode == "top":
        target_pose_obj = np.array([
            [ 0.04467006, -0.99587781,  0.07894284,  0.03156405],
            [-0.99201327, -0.03488824,  0.12121259, -1.04378722],
            [-0.11795875, -0.08372692, -0.98948246,  0.27561569],
            [ 0.        ,  0.,          0.,          1.        ],
        ])
        target_pose_obj = np.array([
            [-0.03961017, -0.99699401, -0.06658819,  0.00209295],
            [-0.99918439,  0.03899749,  0.01047637, -1.0493797 ],
            [-0.0078481,   0.06694885, -0.99772554,  0.27951137],
            [ 0.        ,  0.,          0.,          1.        ],
        ])
        target_pose_obj = np.array([
            [-0.0587541,  -0.99515833, -0.07878999,  0.0114198 ],
            [-0.99287273,  0.05005593,  0.10815796, -1.04499823],
            [-0.10369039,  0.08458315, -0.99100656,  0.25779324],
            [ 0.        ,  0.,          0.,          1.        ],
        ])
        cabinet_pos_world, cabinet_orn_world = scene.object_registry("name", "drawer").get_position_orientation()
        cabinet_pose_world = np.eye(4)
...
save_folder = 'omnigibson/arnav_trial/close_drawer2'
...
drawer.joints[f"j_link_3"].friction = 5000.0
...
while episode_number < init_episode_number + num_samples:
    ...
    drawer.links["link_3"].mass = 50.0
    drawer.links["link_2"].mass = 50.0
    drawer.links["link_1"].mass = 50.0
    drawer.joints[f"j_link_3"].friction = 100.0
    relevant_objs = [drawer]
...
if save_video:
    directory_name = "/home/arpit/projects/OmniGibson/omnigibson/arnav_trial/temp"
    os.makedirs(directory_name, exist_ok=False)
    counter = 0
    for img in data_collection_images:
        cv2.imwrite(os.path.join(directory_name, f"{counter}.jpg"), cv2.cvtColor(img, cv2.COLOR_BGR2RGB))
        counter += 1

    output_video = f"/home/arpit/projects/OmniGibson/omnigibson/arnav_trial/data_collection_videos/close_drawer1.avi"
    create_video_from_images(image_folder=directory_name, output_video=output_video, fps=30)
