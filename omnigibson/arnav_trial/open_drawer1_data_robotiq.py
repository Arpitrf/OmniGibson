...
def custom_reset(env, robot, episode_memory=None, grasp_mode=None): 
    ...
    base_pos = np.array([0.28, 0.0, 0.0])
    if grasp_mode == "front":
...
def grasp_handle(grasp_mode):
    ...
    if grasp_mode == "front":
        target_pose_obj = np.array([
            [-0.05843413, -0.99463599,  0.08535047,  0.0365501],
            [-0.11867914,  0.09181159,  0.98867886, -0.62658401],
            [-0.99121174,  0.04764327, -0.12340748,  0.30764381],
            [ 0.        ,  0.,          0.,          1.        ],
        ])
    elif grasp_mode == "top":
        # w.r.t object (top grasp)
        target_pose_obj = np.array([
            [-0.29600208, -0.94587479, -0.13305509, -0.00647753],
            [-0.95161762,  0.27998926,  0.12660933, -0.65066132],
            [-0.08250258,  0.16409419, -0.98298852,  0.35550842],
            [ 0.        ,  0.,          0.,          1.        ],
        ])
    ...
    if grasp_mode == "top":
        target_pose_obj = np.array([
            [-0.29600208, -0.94587479, -0.13305509, -0.00647753],
            [-0.95161762,  0.27998926,  0.12660933, -0.62066132],
            [-0.08250258,  0.16409419, -0.98298852,  0.30550842],
            [ 0.        ,  0.,          0.,          1.        ],
        ])
        cabinet_pos_world, cabinet_orn_world = scene.object_registry("name", "drawer").get_position_orientation()
        cabinet_pose_world = np.eye(4)
...
save_folder = 'omnigibson/arnav_trial/open_drawer1'
...
for link_number in range(1, 3):
    drawer.links[f"link_{link_number}"].mass = 2.0
    drawer.links[f"link_{link_number}"].friction = 1000.0
