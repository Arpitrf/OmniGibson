...
def main():
    ...
    for i in range (frame_step, hand_positions.shape[0], frame_step):
        ...
        target_hand_orn = torch.tensor(list(R.from_matrix(hand_rotations[i]).as_quat())).to(torch.float32)
        ...
        # delta_hand_orn = torch.from_numpy(delta_hand_orn).to(torch.float32)
    ...
    if save_images:
        output_video = f"/code/omnigibson/arnav_trial/result_videos/{video_name}.avi"
