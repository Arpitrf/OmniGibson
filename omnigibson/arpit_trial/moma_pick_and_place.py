...
def primitive():
    ...
    # move hand to a pose
    target_pose = (th.tensor([0.1829, 0.4876, 0.4351]), th.tensor([-0.0342, -0.0020,  0.9958,  0.0846]))
    # # diagonal 45
    ...
    new_pos = curr_pos + np.array([0.0, 0.0, 0.3])
    ...
    target_base_pose = np.array([0.4256, 0.0257, 0.0])
    execute_controller(action_primitives._navigate_to_pose_direct(target_base_pose), env, robot, gripper_closed)

    input("Press enter to continue =============================")

    # move hand to a pose
    target_pose =  (np.array([ 1.1888, -0.1484,  0.8187]), np.array([-0.0489, -0.0063,  0.5555,  0.8301]))
