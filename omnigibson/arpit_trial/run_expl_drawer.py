...
from utils import correct_gripper_friction, check_success, set_extrinsic_matrix, hori_concatenate_image
...
temp_prior = th.tensor([
    [0.,    0.,    0., -0.055,  -0.043,  -0.020,    0.,    0.,    0., -1.0],
...
def grasp_handle(env, robot, episode_memory=None, action_primitives=None, motion_utils=None):
...
def expl(t, actions, motion_utils, robot, env, traj_length, collision_failure_model=None, grasp_mode=None, grasp_failure_model=None):
    ...
    safe_list = []
    grasp_failure_model_threshold = 0.90
    ...
    for action in actions:
        ...
        if motion_utils.safe(action[t], grasp_failure_model=grasp_failure_model, grasp_mode=grasp_mode, grasp_failure_model_threshold=grasp_failure_model_threshold):
            # In the current implementation I am performing the action (move_primitive) inside the safe action. This will change later.
            all_failed = expl(t+1, actions, motion_utils, robot, env, traj_length, grasp_failure_model=grasp_failure_model, grasp_mode=grasp_mode)
...
def main():
    ...
    grasp_handle(env=env, robot=robot, episode_memory=episode_memory, action_primitives=action_primitives, motion_utils=motion_utils)
