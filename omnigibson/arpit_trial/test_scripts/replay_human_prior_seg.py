...
def slerp(q1, t):
    ...
    Args:
    - q1: Starting quaternion (numpy array of shape [4]).
    - q2: Target quaternion (numpy array of shape [4]).
    ...
    # Ensure shortest path (invert q2 if necessary)
    if dot < 0.0:
        q2 = -q2
        dot = -dot
    ...
    if dot > 0.9995:  # If quaternions are nearly parallel, use linear interpolation
        result = q1 + t * (q2 - q1)
...
def main():
    ...
    # start = 5
    # end = 57
    # init_orn = R.from_matrix(hand_rotations[start]).as_quat()
    # T_hand_ee = np.array([
    #     [0, 0, 1],
    #     [1, 0, 0],
    #     [0, 1, 0]
    # ])
    # for j in range(start, end, frame_step):
    #     curr_orn = T_hand_ee @ hand_rotations[j]
    #     curr_orn = R.from_matrix(curr_orn).as_quat()
    #     prev_orn = T_hand_ee @ hand_rotations[j - frame_step]
    #     prev_orn = R.from_matrix(prev_orn).as_quat()
    #     delta_ori = R.from_quat(curr_orn) * R.from_quat(prev_orn).inv()
    #     # breakpoint()
    #     delta_ori = R.from_quat(delta_ori.as_quat()).as_euler("xyz", degrees=True)
    #     print("j, delta_ori: ", j, delta_ori)
    # breakpoint()
    ...
    for seg_id in range(len(video_seg)):
        ...
        start_body_pos_curr_seg, start_body_yaw_curr_seg = delta_points_init_robot_frame[start], delta_yaws_init_robot_frame[start]
        start_T_body_curr_seg = np.eye(4)
        start_T_body_curr_seg[:3, 3] = start_body_pos_curr_seg
        start_T_body_curr_seg[:3, :3] = R.from_euler("xyz", [0, 0, start_body_yaw_curr_seg]).as_matrix()
        for i in range(start + frame_step, end, frame_step):
            print(f"======== action: {video_seg[str(seg_id)]['description']}, global step {i} ========")
            # 1) moving base
            ...
            if action == "navigating":
                ...
                # 2) moving hand
                current_human_hand_pos, current_human_hand_orn = hand_positions[i], hand_rotations[i]
                prev_human_hand_pos, prev_human_hand_orn = hand_positions[i - frame_step], hand_rotations[i - frame_step]

                # removing the effect of any human body movement by making all hand poses w.r.t initial body pose and not the body pose at each frame
                curr_body_pos_curr_seg, curr_body_yaw_curr_seg = delta_points_init_robot_frame[i], delta_yaws_init_robot_frame[i]
                curr_T_body_curr_seg = np.eye(4)
                curr_T_body_curr_seg[:3, 3] = curr_body_pos_curr_seg
                curr_T_body_curr_seg[:3, :3] = R.from_euler("xyz", [0, 0, curr_body_yaw_curr_seg]).as_matrix()
                T_curr_body_wrt_start_body = np.linalg.inv(start_T_body_curr_seg) @ curr_T_body_curr_seg
                # convert current_human_hand_pose and prev_human_hand_pose from current body frame to start body frame
                T_curr_hand_wrt_curr_body = np.eye(4)
                T_curr_hand_wrt_curr_body[:3, 3] = current_human_hand_pos
                T_curr_hand_wrt_curr_body[:3, :3] = current_human_hand_orn
                T_prev_hand_wrt_curr_body = np.eye(4)
                T_prev_hand_wrt_curr_body[:3, 3] = prev_human_hand_pos
                T_prev_hand_wrt_curr_body[:3, :3] = prev_human_hand_orn
                T_curr_hand_wrt_start_body = T_curr_body_wrt_start_body @ T_curr_hand_wrt_curr_body
                T_prev_hand_wrt_start_body = T_curr_body_wrt_start_body @ T_prev_hand_wrt_curr_body
                current_human_hand_pos, current_human_hand_orn = T_curr_hand_wrt_start_body[:3, 3], T_curr_hand_wrt_start_body[:3, :3]
                prev_human_hand_pos, prev_human_hand_orn = T_prev_hand_wrt_start_body[:3, 3], T_prev_hand_wrt_start_body[:3, :3]


                delta_human_hand_pos = current_human_hand_pos - prev_human_hand_pos
                scale_down = 0.5
                delta_human_hand_pos = np.array([delta_human_hand_pos[2], delta_human_hand_pos[0], delta_human_hand_pos[1]]) * scale_down
                delta_human_hand_pos = delta_human_hand_pos.astype("float32")
                print("delta_human_hand_pos: ", delta_human_hand_pos)

                # transform the hand frame to ee frame
                T_hand_ee = np.array([
                    [0, 0, 1],
                    [1, 0, 0],
                    [0, 1, 0]
                ])
                current_human_hand_orn = T_hand_ee @ current_human_hand_orn
                prev_human_hand_orn = T_hand_ee @ prev_human_hand_orn
                current_human_hand_orn_quat = R.from_quat(R.from_matrix(current_human_hand_orn).as_quat())
                prev_human_hand_orn_quat = R.from_quat(R.from_matrix(prev_human_hand_orn).as_quat())
                delta_human_hand_orn = current_human_hand_orn_quat * prev_human_hand_orn_quat.inv()
                delta_human_hand_orn = delta_human_hand_orn.as_quat()
                # TODO: reduce the magnitude of delta_human_hand_orn
                # delta_human_hand_orn = slerp(delta_human_hand_orn, t=1.0)
                # print("delta_human_hand_orn: ", R.from_quat(delta_human_hand_orn).as_euler("xyz", degrees=True))
                ...
                # breakpoint()
                target_robot_hand_orn = R.from_quat(current_robot_hand_quat) * R.from_quat(delta_human_hand_orn)
                target_robot_hand_orn = th.tensor(target_robot_hand_orn.as_quat(), dtype=th.float32)
                # if don't want to change orientations
    ...
    # for i in range(step_size, end, step_size):
    #     print("======== step: ", i)
    #     # 1) moving base
    #     # convert base delta poses from init robot frame to world frame. Note that the delta_pose is (current_pose - init_pose) and not (current_pose - prev_pose)
    #     delta_pos_world_frame = R_R0_to_world @ np.transpose(delta_points_init_robot_frame[i])
