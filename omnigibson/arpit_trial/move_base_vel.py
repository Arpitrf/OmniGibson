...
def detect_robot_collision_custom(robot):
    # check robot collisions
    robot_collision_1 = detect_robot_collision_in_sim(robot)
    contact_list = robot.links[f"base_link"].contact_list()
    robot_collision_2 = False
    for contact in contact_list:
        if "wall" in contact.body1:
            robot_collision_2 = True
    robot_collision = robot_collision_1 or robot_collision_2
    return robot_collision

# Function to update the robot's pose incrementally
def move_to_target(env, robot, action_primitives, current_pose, target_pose, initial_pose_wrt_map, tf_map=None, pos_tolerance=0.02, orn_tolerance=0.1, writer=None):
    ...
    while True:
        ...
        if abs(d_x) < pos_tolerance and abs(d_y) < pos_tolerance and abs(d_yaw) < orn_tolerance:
            ...
            if sum(robot_collisions) > 3:
                print("Robot collision detected.")
                return {"robot_base_collision": True}
            else:
                return {"robot_base_collision": False}
            
        if step > steps_th:
            print("Reached max steps. Robot probably collisided.")
            return {"robot_base_collision": True}
        ...
        action[robot.base_action_idx] = th.tensor([v_x, v_y, omega_z], dtype=th.float32)
        env.step(action)
        robot_collision = detect_robot_collision_custom(robot)       
        robot_collisions.append(robot_collision)
        ...
        current_pose_wrt_map = T.pose2mat(robot.get_position_orientation())
        # current_pos_map = current_pose_wrt_map[:3, 3]
        # current_ori_map = T.mat2quat(current_pose_wrt_map[:3, :3])
        current_pose_wrt_inital_pose = th.linalg.inv(initial_pose_wrt_map) @ current_pose_wrt_map
...
def move_base_vel(env, robot, action_primitives, action, writer=None):
    d_x, d_y, d_yaw = action
    delta_pos = th.tensor([d_x, d_y, 0.0], dtype=th.float32)
    delta_orn = th.tensor(R.from_euler('xyz', [0.0, 0.0, d_yaw]).as_quat(), dtype=th.float32)
    delta_pose2d = np.array([d_x, d_y, d_yaw])

    # Obtain target base pose in current base pose frame
    initial_pose_wrt_map = T.pose2mat(robot.get_position_orientation())
    ...
    target_pose_wrt_initial_pose = th.linalg.inv(initial_pose_wrt_map) @ target_pose_wrt_map
    ...
    retval = move_to_target(env, robot, action_primitives, initial_pose2d_wrt_initial_pose, target_wrt_initial_pose2d, initial_pose_wrt_map=initial_pose_wrt_map, writer=writer)


    transform = T.pose2mat(robot.get_position_orientation())
