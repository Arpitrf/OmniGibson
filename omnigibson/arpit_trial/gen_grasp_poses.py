...
def obtain_grasp_pose(env, org_grasp_pos_world, org_grasp_quat_world):
    # original robot pose w.r.t world
    org_grasp_matrix_world = R.from_quat(org_grasp_quat_world).as_matrix()
    T_w_g1 = np.vstack((np.column_stack((org_grasp_matrix_world, org_grasp_pos_world)), [0, 0, 0, 1]))
    
    # original object pose w.r.t world
    org_obj_pos_world, org_obj_quat_world = np.array([-0.50075144, -0.7176887, 0.44736257]), np.array([ 2.2960646e-04, -1.9401989e-04, -2.0902123e-01, 9.7791111e-01])
    org_obj_matrix_world = R.from_quat(org_obj_quat_world).as_matrix()
    T_w_o1 = np.vstack((np.column_stack((org_obj_matrix_world, org_obj_pos_world)), [0, 0, 0, 1]))
   
    # new object pose w.r.t world
    scene_state = env.scene.dump_state(serialized=False)
    new_obj_pos_world, new_obj_quat_world = scene_state['object_registry']['box']['root_link']["pos"], scene_state['object_registry']['box']['root_link']['ori']
    new_obj_matrix_world = R.from_quat(new_obj_quat_world).as_matrix()
    # print("box: ", scene_state['object_registry']['box']['root_link'])
    # print("object_pos_world, object_quat_world: ", object_pos_world, object_quat_world)
    T_w_o2 = np.vstack((np.column_stack((new_obj_matrix_world, new_obj_pos_world)), [0, 0, 0, 1]))
    ...
    # # o1 w.r.t o2
    # T_o2_o1 = np.dot(np.linalg.inv(T_w_o2), T_w_o1)

    # # obtain new gripper pose
    # T_w_g2 = np.dot(T_o2_o1, T_w_g1)
    # new_grasp_pos_world, new_grasp_matrix_world = T_w_g2[:3, 3], T_w_g2[:3, :3]
    # new_grasp_quat_world = R.from_matrix(new_grasp_matrix_world).as_quat()

    # MimicGen method
    T_w_g2 = np.dot(np.dot(T_w_o2, np.linalg.inv(T_w_o1)), T_w_g1)
    new_grasp_pos_world, new_grasp_matrix_world = T_w_g2[:3, 3], T_w_g2[:3, :3]
    new_grasp_quat_world = R.from_matrix(new_grasp_matrix_world).as_quat()

    return new_grasp_pos_world, new_grasp_quat_world
...
def grasp_primitive(action_primitives, env, robot, episode_memory, arr):
    ...
    org_grasp_pos_robot, org_grasp_quat_robot = np.array([ 0.50431009, -0.25087801,  0.46123985]), np.array([ 0.57241185,  0.58268626, -0.41505368,  0.4006892 ])
    org_grasp_pos_world, org_grasp_quat_world = np.array([-0.51942176, -0.71130641,  0.46123963]), np.array([ 0.79082703, -0.20438009, -0.55453387, -0.1591024 ])

    # Converting robot frame to world frame
    # org_matrix = R.from_quat(org_grasp_quat_robot).as_matrix()
    # T_r_g = np.vstack((np.column_stack((org_matrix, org_grasp_pos_robot)), [0, 0, 0, 1]))
    # robot_base_pos, robot_base_quat = robot.get_position_orientation()
    # robot_base_matrix = R.from_quat(robot_base_quat).as_matrix()
    # T_w_r = np.vstack((np.column_stack((robot_base_matrix, robot_base_pos)), [0, 0, 0, 1])) 
    # T_w_g = np.dot(T_w_r, T_r_g)
    # sanity check
    # grasp_pos_world, grasp_orn_matrix_world = T_w_g[:3, 3], T_w_g[:3, :3]
    # grasp_orn_quat_world = R.from_matrix(grasp_orn_matrix_world).as_quat()
    # grasp_pose_world = (np.array(grasp_pos_world), np.array(grasp_orn_quat_world))
    # print("grasp_pose_world: ", grasp_pose_world)
    # new_pos, new_quat = grasp_pose_world[0], grasp_pose_world[1]

    new_pos, new_quat = obtain_grasp_pose(env, org_grasp_pos_world, org_grasp_quat_world)
    # new_pos, new_quat = org_grasp_pos_world, org_grasp_quat_world 
    ...
    # new_pos = org_grasp_pos_robot + np.random.uniform(-0.02, 0.02, 3)
    ...
    # org_rotvec = np.array(R.from_quat(org_grasp_quat_robot).as_rotvec())
    ...
    # remove later
    org_grasp_pose_robot = (np.array(org_grasp_pos_robot), np.array(org_grasp_quat_robot))
    
    x = np.random.uniform(org_grasp_pos_robot[0] - 0.2, org_grasp_pos_robot[0] + 0.2)
    y = np.random.uniform(org_grasp_pos_robot[1] - 0.2, org_grasp_pos_robot[1] + 0.2)
    z = np.random.uniform(org_grasp_pos_robot[2] + 0.2, org_grasp_pos_robot[2] + 0.4)
    neighbourhood_pose = (np.array([x, y, z]), org_grasp_pose_robot[1])
...
def main():
    ...
    # Generate a random rotation angle between 0 and 2π (radians) for the z-axis
    random_angle = np.random.uniform(0, np.pi)
    random_angle = np.pi / 2
    # Create a rotation quaternion about the z-axis
    z_rotation = R.from_euler('z', random_angle).as_quat()
    # Combine the z-axis rotation with the original quaternion
    # Note: Quaternion multiplication order matters. Here, we apply z_rotation after the original one.
    original_quaternion = np.array([ 2.2960646e-04, -1.9401989e-04, -2.0902123e-01, 9.7791111e-01])
    new_quaternion = R.from_quat(original_quaternion) * R.from_quat(z_rotation)
    ...
    config["objects"] = [
        {
            ...
            "scale": [0.1, 0.05, 0.1],
            # "size": 0.05,
            "position": [-0.6, -0.7, 0.45], # was [-0.5, -0.7, 0.45]
            "orientation": new_quaternion,
        },
        {
            "type": "DatasetObject",
            "name": "table",
    ...
    save_folder = 'dynamics_model_dataset_seg'
